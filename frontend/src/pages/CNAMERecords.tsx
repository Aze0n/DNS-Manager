// cname-records seite (platzhalter)
import React, { useState, useEffect } from "react";
import { getRecordsForDomain } from "../api/client";

interface CNAMERecordsProps { selectedDomain?: string }
const CNAMERecords: React.FC<CNAMERecordsProps> = ({ selectedDomain }) => {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await getRecordsForDomain("CNAME", selectedDomain);
      ...
      <div className="table-responsive">
        <table className="table table-striped table-sm">
          ...
          <tbody>
            {records.length === 0 && (
              <tr>
                <td colSpan={6} className="text-muted">noch keine daten</td>
              </tr>
            )}
            {records.map((r, idx) => (
              <tr key={idx}>
          ...
          <div className="modal d-block" tabIndex={-1}>
            <div className="modal-dialog">
                ...
                <div className="modal-header">
                  <h5 className="modal-title">neuer CNAME-Eintrag</h5>
                ...
                <div className="modal-body">
                  <form onSubmit={async (e) => {
                    ...
                    if (!content) { alert('Ziel darf nicht leer sein'); return; }
                    ...
                    setLoading(true);
                    try {
                      const payload: any = { type: 'CNAME', name, content };
                      ...
                      setRecords(prev => [{ domain: selectedDomain, name, type: 'CNAME', content, ttl }, ...prev]);
                      setShowModal(false);
                    ...
                    <div className="mb-3">
                      ...
                      <input name="content" value={modalContent} onChange={e => setModalContent(e.target.value)} type="text" className="form-control" placeholder="target.example.com" required />
                    </div>
                    <div className="mb-3">
                      <label className="form-label">TTL (Sekunden)</label>
                      <input name="ttl" value={modalTTL ?? ''} onChange={e => setModalTTL(e.target.value)} type="number" className="form-control" placeholder="3600" />
                    </div>
